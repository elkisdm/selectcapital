import { PROJECT_CONFIG } from './project-data.js';
...
const renderHero = () => {
  ...
  if (subtitleNode) {
    subtitleNode.textContent = `Entrega ${project.delivery} · Pie en cuotas · Asesoría sin costo`;
  }
  ...
  if (heroMedia) {
    ...
    overlay.innerHTML = `<strong>${project.name}</strong><p>${project.commune} · Entrega ${project.delivery}</p><p>Etapa ${project.stage}</p>`;
...
const renderTypologies = () => {
  ...
  state.config.typologies?.forEach((typology) => {
    ...
    priceRow.innerHTML = `<span>UF referencial</span><strong>UF ${ufFormatter.format(typology.ufPrice)}</strong>`;
    metrics.appendChild(priceRow);

    const areaRow = document.createElement('span');
    areaRow.innerHTML = `<span>Sup. total</span><strong>${typology.areaM2} m²</strong>`;
    metrics.appendChild(areaRow);

    const downRow = document.createElement('span');
    downRow.innerHTML = `<span>Pie</span><strong>UF ${ufFormatter.format(
      typology.downPaymentUF
    )} · ${typology.downPaymentInstallments} cuotas</strong>`;
...
const renderAccompaniment = () => {
  ...
  if (badge) {
    badge.textContent = 'Asesoría sin costo · Sin comisión al comprador';
  }
};
...
const renderLegal = () => {
  ...
  if (legalNotes) {
    const notes = [
      'Valores referenciales. Sujetos a evaluación crediticia.',
      'La disponibilidad de unidades puede variar sin previo aviso.',
